...
check_pattern_present "lib/api.sh" '\\"smtpPort\\":587' "api.sh sends valid SMTP port for 3x-ui settings validation"
check_pattern_present "lib/api.sh" '\\"smtpEncryptionType\\":\\"starttls\\"' "api.sh sends SMTP encryption type for 3x-ui settings validation"
check_pattern_present "lib/api.sh" '\\"fingerprint\\"[[:space:]]*:[[:space:]]*\\"firefox\\"' "api.sh uses explicit Firefox Reality fingerprint"
check_pattern_absent "lib/api.sh" '\\"fingerprint\\"[[:space:]]*:[[:space:]]*\\"random\\"' "api.sh does not use random Reality fingerprint"
